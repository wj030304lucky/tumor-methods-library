...
function render(){
 ...
 <div class="paper-actions" onclick="event.stopPropagation()"><select style="width:auto" onchange="setState('${p.id}',{status:this.value})"><option value="unread" ${x.status==='unread'?'selected':''}>未读</option><option value="reading" ${x.status==='reading'?'selected':''}>在读</option><option value="done" ${x.status==='done'?'selected':''}>已读</option></select><button class="btn ghost favorite-btn ${x.favorite?'active':''}" onclick="setState('${p.id}',{favorite:${!x.favorite}})">${x.favorite?'★ 已收藏':'☆ 收藏'}</button><button class="btn" onclick="location.href='detail.html?id=${encodeURIComponent(p.id)}'">查看详细方法学</button></div></article>`}).join('');
